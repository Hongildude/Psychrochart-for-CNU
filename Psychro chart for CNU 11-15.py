...
def callback_plus1():
    ...
    for key, val in line_dic.items():
        ax2.plot([dot_dic[val[0]][0], dot_dic[val[1]][0]], [dot_dic[val[0]][2], dot_dic[val[1]][2]])
    ax2.axis('off')
...
def tool_option():
    ...
    def callback2_tool(var):
        if CheckVar2_tool.get() == 1:
            ent1_tool.config(state='normal')
            ent1_tool.delete(0, END)
            ent1_tool.insert(0, ((101325 - float(var.get()) * 1000) / rho) / g)
            ent1_tool.config(state='disable')
            if float(var.get()) < 66.045 or float(var.get()) > 101.325:
                messagebox.showerror("error", "범위를 벗어났습니다(66.045~101.325)", parent=new_win)
                ent2_tool.delete(0, END)
                ent2_tool.insert(0, 66.045)
                ent1_tool.config(state='normal')
                ent1_tool.delete(0, END)
                ent1_tool.insert(0, ((101325 - pressure) / rho) / g)
                ent1_tool.config(state='disable')
        return
    ...
    ent1_tool.delete(0, END)
    ent1_tool.insert(0, custom_style['limits']['altitude_m'])
...
menu_tool.add_command(label="인쇄", command=tool_print)
